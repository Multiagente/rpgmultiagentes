...
/**
 * This class is a specialization of the abstract class Agent.It holds the
 * static and behavioral properties of the game's character.
 */
public class CharacterAgent extends Agent {
	...
	public void setPosition( Square currentSquare ) {
		this.currentPosition = currentSquare;
	}
	...
	private void exitRoom() {
		...
		List<AID> listCharacters = getPosition().charactersInside();
		
		if( listCharacters.size() > 1 ) {
			Iterator<AID> iterator = listCharacters.iterator();
			
			while( iterator.hasNext() ) {
				AID receiver = iterator.next();
	...
	private void fight( AID target ) {
		...
		addBehaviour( new AchieveREInitiator( this, msg ) {
			...
			protected void handleAllResultNotifications( Vector notifications ) {
				// System.out.println("[FIGHT:"+getLocalName()+"] Timeout expired: missing "
				// + (notifications.size()) + " responses");
			}
		} );
	}
	...
	private void updatePosition( Square newPosition ) throws FIPAException {
		...
		System.out.println( newPosition.getDescription() );
	}
	...
	private class HandleMoveRequest extends OneShotBehaviour {
		...
		public void action() {
			...
			mySelf.addBehaviour( new AchieveREResponder( mySelf, template ) {
				...
				protected ACLMessage prepareResponse( ACLMessage request )
					...
					if( checkAction() ) {
						/*
						 * We agree to perform the action. Note that in the
						 * FIPA-Request protocol the AGREE message is optional. Return
						 * null if you don't want to send it.
						 */
		...
		private boolean checkAction() {
			/*
			 * Adicionar verificação para as possíveis ações dos agentes,
			 * casos estejam ocupados. Simulate a check by generating a random
			 * number.
			 */
		...
		public void action() {
			...
			mySelf.addBehaviour( new AchieveREResponder( mySelf, template ) {
				...
				protected ACLMessage prepareResultNotification( ACLMessage request,
					...
					System.out.println( "[" + getLocalName() + "]{HP:"
					      + getHealthPoints() + "} Received a hit from ["
					      + request.getSender().getLocalName() + "]{HIT:"
					      + request.getContent() + "}" );
					ACLMessage inform = request.createReply();
					inform.setPerformative( ACLMessage.INFORM );
